...
export function GoogleAuth({ className }: GoogleAuthProps) {
  ...
  // Get callbackUrl from URL or use dashboard as default
      ...
      if (error) {
        ...
        if (error.code === "OAUTH_ERROR") {
          ...
          toast.error("Too Many Attempts", {
            description: "You've made too many authentication attempts. Please try again later.",
          });
        } else {
  ...
  return (
    <Button
      ...
      disabled={isLoading}
      className={cn(
        "relative h-12 px-6 text-base font-medium",
